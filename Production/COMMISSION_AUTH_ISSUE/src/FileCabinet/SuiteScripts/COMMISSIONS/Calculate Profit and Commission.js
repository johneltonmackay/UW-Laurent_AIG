...
define(['N/record', 'N/search', 'N/log', 'N/format', 'N/runtime'], function(record, search, log, format, runtime) {
    ...
    function beforeLoad(context) {
        let objCurrRec = context.newRecord;
        let form = context.form;

        let fieldIdsToHide = ['custbody_profit', 'custbody_commission_amount'];
        let fieldIdsToNull = ['custbody_boldocument', 'custbody_tenderattachment'];
        
        fieldIdsToHide.forEach(function (fieldId) {
            let field = form.getField({ id: fieldId });
            if (field) {
        ...
        if (context.type === context.UserEventType.COPY || context.type === context.UserEventType.CREATE) {
            fieldIdsToNull.forEach(function (fldId) {
                objCurrRec.setValue({ fieldId: fldId, value: null });
            });
        }
        ...
        try {
            ...
            if (commissionAuthId){
                intSOId = record.submitFields({
                    ...
                    id: salesOrder.id,
                    values: {
                        'custbody_commission_auth_id': commissionAuthId,
                    },
                    options: {
                        enableSourcing: true,
