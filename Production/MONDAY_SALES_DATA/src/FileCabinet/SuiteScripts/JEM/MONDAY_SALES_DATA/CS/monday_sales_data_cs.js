 ...
 */
define(['N/currentRecord', 'N/url', '../Library/monday_sales_data_sl_mapping.js', 'N/search', 'N/ui/message', 'N/record', 'N/runtime'],
    
    (currentRecord, url, mondayMapping, search, message, record, runtime) => {
        ...
        try {
            ...
            if (isCloseWindow) {
                ...
                if (arrjsonData) {
                    ...
                    setDepartment(objCurrentRecord, objForm)
                }
            }
    ...
    const setDepartment = (objCurrentRecord, objForm) => {
        let currentUser = runtime.getCurrentUser();
        console.log('setDepartment Current User Department', currentUser);
        if (currentUser.id == -5){
            let strField = objCurrentRecord.getField({ fieldId: 'custpage_department_head' })
            strField.isDisabled = false
            console.log('setDepartment strField', strField)
        } else {
            objCurrentRecord.setValue({
                fieldId: 'custpage_department_head',
