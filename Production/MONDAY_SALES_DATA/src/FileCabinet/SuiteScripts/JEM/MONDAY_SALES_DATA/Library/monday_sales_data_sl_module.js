                        ...
                        if (value !== undefined && value !== null && value !== "") {
                            try {
                                if (key == 'custpage_load_order'){
                                    ...
                                    let strFrom = data['custpage_pu_city_state'];
                                    let strTo = data['custpage_drop_city_state'];
        ...
        const runSearch = (options) => {
            try {
                ...
                let stdColumns = [
                    ...
                    search.createColumn({ name: 'custbody_pu_city_state', join: 'custrecord_load_order', label: 'custpage_pu_city_state' }),
                    search.createColumn({ name: 'custbody_drop_city_state', join: 'custrecord_load_order', label: 'custpage_drop_city_state' }),
                ...
                if (puLocation) {
                    ...
                    customFilters.push(['custrecord_load_order.custbody_pu_city_state', 'contains', puLocation]);
                }

                if (dropLocation) {
                    if (customFilters.length > 0) customFilters.push('AND');
                    customFilters.push(['custrecord_load_order.custbody_drop_city_state', 'contains', dropLocation]);
                }
        ...
        const runSearch2nd = (options) => {
            try {
                ...
                let customFilters = [
                    ['type', 'anyof', 'PurchOrd'],
                    'AND',
                    ['status', 'noneof', 'PurchOrd:G', 'PurchOrd:H', 'PurchOrd:C'],
                    'AND',
                    ['mainline', 'is', 'T'],
                    'AND',
                ...
                let stdColumns = [
                    ...
                    search.createColumn({ name: 'custbody_pu_city_state', label: 'custpage_pu_city_state' }),
                ]
                ...
                if (arrSearchParam.length > 0){
                    ...
                    if (arrSearchParam[0].custpage_drop_location_head){
                        customFilters.push('AND'),
                        customFilters.push(['custbody_drop_city_state', 'contains', arrSearchParam[0].custpage_drop_location_head])
                    }
                    if (arrSearchParam[0].custpage_pu_location_head){
                        customFilters.push('AND'),
                        customFilters.push(['custbody_pu_city_state', 'contains', arrSearchParam[0].custpage_pu_location_head])
                    }
                }
